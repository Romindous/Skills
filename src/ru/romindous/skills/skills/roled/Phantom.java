...
public class Phantom implements Scroll.Regable {
    @Override
    public void register() {
        ...
        new Ability() {//Диспульсия
            ...
            final ChasMod DAMAGE = new ChasMod(this, "hurt", Chastic.DAMAGE_TAKEN);
            ...
            public boolean cast(final Chain ch, final int lvl) {
                ...
                if (EntityUtil.center(caster).distanceSquared(EntityUtil.center(tgt)) > dst * dst) {
            ...
            private final String[] desc = new String[] {
                TCUtil.N + "Отражает полученый " + CLR + "урон " + TCUtil.N + "в цель,",
                TCUtil.N + "если она ближе " + DIST.id() + " бл. " + TCUtil.N + "Из за",
                TCUtil.N + "отскока, урон снижается на " + DAMAGE.id() + "x"};
            public String[] descs() {
                return desc;
            }
            public Rarity rarity() {
        ...
        new Ability() {//Путаница
            ...
            public boolean cast(final Chain ch, final int lvl) {
                ...
                final LivingEntity caster = ch.caster();
                final Location loc = tgt.getLocation();
                ...
                tgt.setTarget(le);
                addEffect(tgt, PotionEffectType.BLINDNESS, 100d, 1, false);
                if (le instanceof final Mob tm) {
                    tm.setTarget(tgt);
                    addEffect(tm, PotionEffectType.BLINDNESS, 100d, 1, false);
                }

                EntityUtil.effect(tgt, Sound.ENTITY_ILLUSIONER_CAST_SPELL,
                    0.8f, Particle.CAMPFIRE_COSY_SMOKE);
