...
public class Medaline extends ItemGroup {
    private int chance;
    private float gold_speed_mul;
    private float gold_dur_mul;
    private double thorns;

    Medaline(final ItemStack... its) {
        super(its);
    }

    public void before() {
        chance = SkillGroups.itemConfig(this, "chance", 2);
        gold_speed_mul = (float) SkillGroups.itemConfig(this, "gold_speed_mul", 0.8d);
        gold_dur_mul = (float) SkillGroups.itemConfig(this, "gold_dur_mul", 1.4d);
        thorns = SkillGroups.itemConfig(this, "thorn_dmg", 1.2d);
    }
    ...
    protected void onAttack(final EquipmentSlot[] es, final EntityDamageByEntityEvent e) {
        ...
        if (DamageLst.DIRECT.contains(ds.getDamageType())
            && SkillGroups.has(es, EquipmentSlot.HAND) && Roll.roll(chance)) {
