...
public class Serebrite extends ItemGroup {
    private int chance;
    private int arm_tough;
    private float iron_dur_mul;
    private float iron_speed_mul;

    Serebrite(final ItemStack... its) {
        super(its);
    }

    public void before() {
        chance = SkillGroups.itemConfig(this, "chance", 4);
        arm_tough = SkillGroups.itemConfig(this, "arm_tough", 1);
        iron_dur_mul = (float) SkillGroups.itemConfig(this, "iron_dur_mul", 0.8d);
        iron_speed_mul = (float) SkillGroups.itemConfig(this, "iron_speed_mul", 1.2d);
    }
    ...
    protected void onAttack(final EquipmentSlot[] es, final EntityDamageByEntityEvent e) {
        ...
        if (DamageLst.DIRECT.contains(ds.getDamageType()) && SkillGroups.has(es, EquipmentSlot.HAND) && Roll.roll(chance)
