...
public class Rotting extends SednaMob {
    ...
    public Map<EquipmentSlot, ItemStack> equipment() {
        return Map.of(EquipmentSlot.HAND, ClassUtil.rndElmt(
            ...
            EquipmentSlot.CHEST, ClassUtil.rndElmt(SkillMats.MEDAL.item(ItemType.GOLDEN_CHESTPLATE), ItemUtil.air,
                new ItemBuilder(Material.LEATHER_CHESTPLATE).color(Color.ORANGE).build(), ItemUtil.air),
            EquipmentSlot.LEGS, ClassUtil.rndElmt(SkillMats.MEDAL.item(ItemType.GOLDEN_LEGGINGS), ItemUtil.air, ItemUtil.air,
                new ItemBuilder(Material.LEATHER_LEGGINGS).color(Color.ORANGE).build(), ItemUtil.air),
            EquipmentSlot.FEET, ClassUtil.rndElmt(SkillMats.MEDAL.item(ItemType.GOLDEN_BOOTS), ItemUtil.air, ItemUtil.air,
                new ItemBuilder(Material.LEATHER_BOOTS).color(Color.ORANGE).build(), ItemUtil.air));
    }
    ...
    private final RollTree drop = RollTree.of(key().value())
        .add(new ItemRoll(key().value() + "_flesh", new ItemBuilder(ItemType.ROTTEN_FLESH).build(), 2, 1), 4)
        .add(new ItemRoll(key().value() + "_nugget", SkillMats.MEDAL.item(ItemType.GOLD_NUGGET), 1, 1, 3), 2)
        .add(new ItemRoll(key().value() + "_ingot", new ItemBuilder(ItemType.COPPER_INGOT).build(), 2, 1), 1)
        .add(new ItemRoll(key().value() + "_pork", new ItemBuilder(ItemType.PORKCHOP).build(), 1, 1), 1)
        .build(1, 2);
