...
public enum Stat {
    ...
    ACCURACY("Точность", "<mint>", ItemType.TARGET), //cross - ranged dmg, exp
    ...
    public ItemStack getItem(final Survivor sv) {
        return new ItemBuilder(icon).name(TCUtil.sided("<u>" + disName()
            + " " + sv.getStat(this) + "</u>", "❖")).lore(getDesc(sv)).build();
    }
    ...
    private void numDesc(final String name, final double rt, final double nrt,
        ...
        if (next) {
            if (add) {
                dscs.add(fin + TCUtil.N + " на " + clr + StringUtil.toSigFigs(num * rt, SIG_FIGS_NUM)
                    + TCUtil.P + " (-> " + StringUtil.toSigFigs(num * nrt, SIG_FIGS_NUM) + ")");
            } else {
                dscs.add(fin + TCUtil.N + " на " + clr + StringUtil.toSigFigs(100d * num * rt, SIG_FIGS_PER) + "%"
                    + TCUtil.P + " (-> " + StringUtil.toSigFigs(100d * num * nrt, SIG_FIGS_PER) + "%)");
            }
        } else {
            if (add) {
                dscs.add(fin + TCUtil.N + " на " + clr + StringUtil.toSigFigs(num * rt, SIG_FIGS_NUM));
    ...
    private void numDesc(final String name, final double rt, final double nrt,
        ...
        if (next) {
            dscs.add(fin);
            dscs.add(TCUtil.N + "на " + clr + StringUtil.toSigFigs(add * rt, SIG_FIGS_NUM)
                + TCUtil.P + " (-> " + StringUtil.toSigFigs(add * nrt, SIG_FIGS_NUM) + ")"
                + TCUtil.N + " или " + clr + StringUtil.toSigFigs(100d * mul * rt, SIG_FIGS_PER) + "%"
                + TCUtil.P + " (-> " + StringUtil.toSigFigs(100d * mul * nrt, SIG_FIGS_PER) + "%)");
        } else {
            dscs.add(fin + TCUtil.N + " на " + clr + StringUtil.toSigFigs(add * rt, SIG_FIGS_NUM)
                + TCUtil.N + " или " + clr + StringUtil.toSigFigs(100d * mul * rt, SIG_FIGS_PER) + "%");
        }
    ...
    private static final int del = ConfigVars.get("stats.rootDel", 4);
